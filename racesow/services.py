...
def map_get_rec_value(num_completed_races, races, best_race):
    """Compute the number of points to award the 1st player.

    It considers the amount of playtime other players spent on the map, the
    general idea being: more playtime && more players --> more points for
    1st place.

    :param num_completed_races: number of Race objects with a time
    :param races: array of Race objects, sorted by playtime (desc)
    :return: points to award to 1st place (value between MIN_REC_POINTS and
        MAX_REC_POINTS)
    """
    ...
    for race in races:
        if race.pk == best_race.pk:
            # The fastest player cannot increase his own score by playing
            # longer. On the contrary: the more he plays, the more he bumps
            # the score in case another player takes the rec. As points are
            # always calculated from scratch, the rec will be worth less when
            # he re-recs because his own playtime is once again excluded.
            continue
...
def _update_points(race, new_points, race_rank, reset):
    ...
    if reset:
        # race & player points were reset, update player's total points and
        # maps_finished
        race.player.add_points(new_points)
        race.player.maps_finished += 1
    else:
        if not floats_differ(old_points, -1):
            # race did not have points yet, increment finished maps and add
            # points to player total
            race.player.add_points(new_points)
            race.player.maps_finished += 1
        elif floats_differ(new_points, old_points):
            # race did have points, different from the newly computed value;
            # subtract old value from player total
...
def map_evaluate_points(mid, reset):
    ...
    completed_races = Race.objects.filter(map__id=mid, time__isnull=False) \
                                  .order_by('time')
    ...
    all_races = Race.objects.filter(map__id=mid) \
                            .order_by('-playtime') \
    ...
    # average of top 20 racetimes
    top20avg = average([race.time for race in completed_races[:20]])

    # cap 2nd place by percentage, with minimum point difference of 2
    second_place_cap = min(rec_points - 2, rec_points * SECOND_PLACE_PERC)
    x = second_place_cap * (7 / 9)
    first_time = best_race.time

    # compute points for 2nd place (capped to a percentage of 1st place)
    capdiff = x * ((completed_races[1].time - first_time) / (top20avg * 0.8))
    second_place_points = min(second_place_cap, second_place_cap - capdiff)
    ...
    # award points for 3rd, 4th... place by differentiating their times with
    # first place, with a minimum of 2 points to the previous time
    points_above = second_place_points
    for rank, race in enumerate(completed_races[2:]):
        capdiff = x * ((race.time - first_time) / (top20avg * 0.8))
        calculated_points = max(
            0,
            min(points_above - 2, second_place_cap - capdiff)
        )
...
def get_next_computation_date():
    """Return date for next task execution

    Returns the next execution of racesow.tasks.recompute_update_maps
    according to its interval schedule

    :return: None|str
    """
    taskname = 'racesow.tasks.recompute_updated_maps'
    last_task = TaskState.objects.filter(name=taskname) \
                                 .order_by('-tstamp')
    if not last_task:
        return None
    ...
    try:
        ...
        for ptask in PeriodicTask.objects.filter(task=taskname):
            try:
                sched = IntervalSchedule.objects.get(pk=ptask.interval_id)
                time_delta = datetime.timedelta(**{sched.period: sched.every})
