...
from racesowold.serializers import raceSerializer as raceoldSerializer
from racesowold.serializers import playerSerializer as playeroldSerializer
...
class APIMapList(View):
    ...
    def get(self, request):
        ...
        # load the tags
        try:
            pattern = _b64decode(request.GET['pattern'])
            tags = json.loads(_b64decode(request.GET['tags']))
        except:
...
class APIMap(View):
    ...
    def post(self, request, b64name):
        ...
        try:
            ...
            tags = json.loads(_b64decode(request.POST['tags']))
        except:
            data = json.dumps({'error': 'Missing parameters'})
            return HttpResponse(data, content_type='application/json',
                                status=400)
...
class APIPlayer(View):
    ...
    def post(self, request, b64name):
        ...
        try:
            ...
            race, created = Race.objects.get_or_create(player=player,
                                                       map_id=mid)
        except:
            data = json.dumps(
                {'error': 'Could not make race for user/map combination'})
            return HttpResponse(data, content_type='application/json',
                                status=400)
        ...
        try:
            ...
            data = json.dumps(
                {'error': 'Could not find map with id {}'.format(mid)})
            return HttpResponse(data, content_type='application/json',
                                status=400)
        ...
        try:
            ...
            # set negative points to indicate that this race is not yet
            # processed
            ...
            # trigger computation of points in next scheduled task
...
class APIRace(View):
    ...
    def get(self, request):
        ...
        races = Race.objects.filter(map=map_, time__isnull=False) \
                            .order_by('time')[:limit]
        data = {
            "map": mapname,
    ...
    def post(self, request):
        ...
        if clear_oneliner:
            # remove old oneliner
            map_.oneliner = ""
            # trigger computation of points for this map in next scheduled
            # task
            map_.compute_points = True
            map_.save()

        # No checks that the race is faster. That should be done on the
        # gameserver
        ...
        # set negative points to indicate that this race is not yet processed
        race.set_points(-1)
        ...
        cp = lambda i, t: Checkpoint(race_id=race.id, number=i, time=t)
        Checkpoint.objects.bulk_create([
            cp(i, t) for i, t in enumerate(checkpoints)
        ])
...
class APIRaceAll(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            data = json.dumps(
                {'error': 'Could not find 1.0 map \'{}\''.format(mapname)})
            return HttpResponse(data, content_type='application/json',
                                status=400)
        ...
        oldraces = PlayerMap.objects.filter(**flt) \
                                    .order_by('time')[:limit]
